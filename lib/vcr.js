...
var VCRjs = function() {
  this.hookInto = null;
  this.cassetteLibraryDir = 'cassettes';
};
...
VCRjs.Context = function(name, cassetteLibraryDir, hookInto) {
  var self = this;

  this.hookInto = hookInto.length > 1 ? hookInto : [hookInto];
  this.cassetteLibraryDir = cassetteLibraryDir;
  this.name = name;
  this.type = null;
  this.url = null;
  ...
  this.getCassetteFor = function(type, url) {
    var cassette = VCRjs.Store.get(self.cassetteLibraryDir, self.name);
  ...
  this.setCassetteFor = function(type, url, response) {
    ...
    VCRjs.Store.set(self.cassetteLibraryDir, self.name, self.cassette);
  }
};
...
VCRjs.prototype = {
  ...
  useCassette: function(name, fn) {
    fn(new VCRjs.Context(name, this.cassetteLibraryDir, this.hookInto));
  }
};
